  ...
  describe 'Settings' do
    ...
    it 'should provide getters and settings' do
      ...
      accessors.each do |accessor|
        value = Kernel.rand
        @settings.send(:"#{accessor}=", value)

        assert_equal value, @settings.send(accessor)

        @settings.send(:"#{accessor}=", nil)
    ...
    it 'configure attribute service attributes correctly' do
      ...
      assert_equal true, @settings.attribute_consuming_service.configured?
      assert_equal 'Test Service', @settings.attribute_consuming_service.name
      assert_equal [{ name: 'Name', name_format: 'Name Format', friendly_name: 'Friendly Name' }], @settings.attribute_consuming_service.attributes
    end
    ...
    it 'does not modify default security settings' do
      ...
      assert_equal false, new_settings.security[:authn_requests_signed]
    ...
    it 'overrides only provided security attributes passing a second parameter' do
      ...
      assert_equal true, @settings.security[:metadata_signed]
    ...
    describe '#get_idp_cert' do
      ...
      it 'returns the certificate when it is valid' do
        ...
        assert_kind_of OpenSSL::X509::Certificate, @settings.get_idp_cert
      end

      it 'raises when the certificate is invalid' do
    ...
    describe '#get_idp_cert_multi' do
      ...
      it 'returns the hash with certificates when values were valid' do
        ...
        assert_kind_of Hash, @settings.get_idp_cert_multi
        assert_kind_of Array, @settings.get_idp_cert_multi[:signing]
        assert_kind_of Array, @settings.get_idp_cert_multi[:encryption]
        assert_kind_of OpenSSL::X509::Certificate, @settings.get_idp_cert_multi[:signing][0]
        assert_kind_of OpenSSL::X509::Certificate, @settings.get_idp_cert_multi[:encryption][0]
      end
      ...
      it 'raises when a cert in idp_cert_multi is not valid' do
    ...
    describe '#get_sp_cert' do
      ...
      it 'returns the certificate when it is valid' do
        ...
        assert_kind_of OpenSSL::X509::Certificate, @settings.get_sp_cert
      end
      ...
      each_key_algorithm do |sp_cert_algo|
        it "allows a certificate with a #{sp_cert_algo.upcase} private key" do
          @settings.certificate, = CertificateHelper.generate_cert(sp_cert_algo).to_pem
    ...
    describe '#get_sp_cert_new' do
      ...
      it 'returns the certificate when it is valid' do
        ...
        assert_kind_of OpenSSL::X509::Certificate, @settings.get_sp_cert_new
      end
    ...
    describe '#get_sp_key' do
      ...
      it 'returns the private key when it is valid' do
        ...
        assert_kind_of OpenSSL::PKey::RSA, @settings.get_sp_key
      end
      ...
      each_key_algorithm do |sp_cert_algo|
        it "allows a #{sp_cert_algo.upcase} private key" do
          ...
          assert_kind_of expected_key_class(sp_cert_algo), @settings.get_sp_key
        end
      end
    end

    describe '#get_fingerprint' do
      it 'gets the fingerprint value when cert and fingerprint in settings are nil' do
      ...
      it 'gets the fingerprint value when there is a cert at the settings' do
        @settings.idp_cert_fingerprint = nil
        @settings.idp_cert = ruby_saml_cert_text
        fingerprint = @settings.get_fingerprint

        assert_equal fingerprint.downcase, ruby_saml_cert_fingerprint.downcase
      end

      it 'gets the fingerprint value when there is a fingerprint at the settings' do
      ...
      it 'gets the fingerprint value when there are cert and fingerprint at the settings' do
        @settings.idp_cert_fingerprint = ruby_saml_cert_fingerprint
        @settings.idp_cert = ruby_saml_cert_text
        fingerprint = @settings.get_fingerprint
    ...
    describe '#get_sp_certs (base cases)' do
      ...
      it 'returns certs for single case' do
        ...
        assert_equal %i[signing encryption], actual.keys
      ...
      each_key_algorithm do |sp_cert_algo|
        it "sp_cert_multi allows #{sp_cert_algo.upcase} certs and private keys" do
          ...
          actual = @settings.get_sp_certs
          expected_signing = @settings.sp_cert_multi[:signing].map(&:values)
          expected_encryption = @settings.sp_cert_multi[:encryption].map(&:values)
      ...
      it 'raises an error when sp_cert_multi is not a Hash' do
      ...
      it 'raises an error when sp_cert_multi does not contain an Array of Hashes' do
      ...
      it 'raises an error when sp_cert_multi inner node missing :certificate' do
      ...
      it 'raises an error when sp_cert_multi inner node missing :private_key' do
      ...
      it 'raises an error when both sp_cert_multi and certificate are specified' do
      ...
      it 'raises an error when both sp_cert_multi and certificate_new are specified' do
        @settings.sp_cert_multi = { signing: [{ certificate: cert_text1, private_key: key_text1 }] }
        @settings.certificate_new = cert_text2
      ...
      it 'raises an error when both sp_cert_multi and private_key are specified' do
        @settings.sp_cert_multi = { signing: [{ certificate: cert_text1, private_key: key_text1 }] }
        @settings.private_key = key_text1
    ...
    describe '#get_sp_certs' do
      ...
      it 'raises an error when all certificates are expired in signing and check_sp_cert_expiration is true' do
        @settings.security = { check_sp_cert_expiration: true }
      ...
      it 'raises an error when all certificates are expired in encryption and check_sp_cert_expiration is true' do
        @settings.security = { check_sp_cert_expiration: true }
    ...
    describe '#get_sp_signing_pair and #get_sp_signing_key' do
      ...
      it 'raises an error when all certificates are expired and check_sp_cert_expiration is true' do
        @settings.security = { check_sp_cert_expiration: true }
    ...
    describe '#get_sp_signature_method' do
      describe 'assumes RSA when sp_cert is nil' do
        ...
        it 'raises an error when digest method is invalid' do
