...
class AuthrequestTest < Minitest::Test
  describe 'Authrequest' do
    ...
    it 'create the deflated SAMLRequest URL parameter' do
      ...
      assert_match(%r{^http://example\.com\?SAMLRequest=}, auth_url)
    ...
    it 'create the deflated SAMLRequest URL parameter including the Destination' do
      ...
      assert_match(%r{<samlp:AuthnRequest[^<]* Destination="http://example\.com"}, inflated)
    end
    ...
    it 'create the SAMLRequest URL parameter with Subject' do
      ...
      assert_includes inflated, '<saml:Subject>'
      assert_includes inflated, '<saml:NameID Format="urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress">testuser@example.com</saml:NameID>'
      assert_includes inflated, '<saml:SubjectConfirmation Method="urn:oasis:names:tc:SAML:2.0:cm:bearer"/>'
    end
    ...
    it 'RelayState cases' do
      ...
      refute_includes auth_url, 'RelayState'

      auth_url = RubySaml::Authrequest.new.create(settings, { RelayState: 'http://example.com' })

      assert_includes auth_url, '&RelayState=http%3A%2F%2Fexample.com'
    ...
    describe 'when the target url contains a query string' do
      it 'create the SAMLRequest parameter correctly' do
        ...
        assert_match(%r{^http://example\.com\?field=value&SAMLRequest}, auth_url)
      end
    end
    ...
    it 'create the saml:AuthnContextClassRef element correctly' do
      ...
      assert_match(%r{<saml:AuthnContextClassRef>secure/name/password/uri</saml:AuthnContextClassRef>}, auth_doc.to_s)
    end
    ...
    it 'create multiple saml:AuthnContextClassRef elements correctly' do
      ...
      assert_match(%r{<saml:AuthnContextClassRef>secure/name/password/uri</saml:AuthnContextClassRef>}, auth_doc.to_s)
      assert_match(%r{<saml:AuthnContextClassRef>secure/email/password/uri</saml:AuthnContextClassRef>}, auth_doc.to_s)
    end
    ...
    it 'create the saml:AuthnContextClassRef with comparison minimum' do
      settings.authn_context = 'secure/name/password/uri'
      settings.authn_context_comparison = 'minimum'
      auth_doc = RubySaml::Authrequest.new.create_authentication_xml_doc(settings)

      assert_match(/<samlp:RequestedAuthnContext[\S ]+Comparison="minimum"/, auth_doc.to_s)
      assert_match(%r{<saml:AuthnContextClassRef>secure/name/password/uri</saml:AuthnContextClassRef>}, auth_doc.to_s)
    end
    ...
    it 'create the saml:AuthnContextDeclRef element correctly' do
      ...
      assert_match(%r{<saml:AuthnContextDeclRef>urn:oasis:names:tc:SAML:2\.0:ac:classes:PasswordProtectedTransport</saml:AuthnContextDeclRef>}, auth_doc.to_s)
    end
    ...
    it 'create multiple saml:AuthnContextDeclRef elements correctly ' do
      ...
      assert_match %r{<saml:AuthnContextDeclRef>name/password/uri</saml:AuthnContextDeclRef>}, auth_doc.to_s
      assert_match %r{<saml:AuthnContextDeclRef>example/decl/ref</saml:AuthnContextDeclRef>}, auth_doc.to_s
    end
