...
class RequestTest < Minitest::Test
  describe 'Logoutrequest' do
    ...
    it 'creates the deflated SAMLRequest URL parameter' do
      ...
      assert_match(%r{^http://unauth\.com/logout\?SAMLRequest=}, unauth_url)
    ...
    it 'RelayState cases' do
      ...
      refute_includes unauth_url, 'RelayState'

      unauth_url = RubySaml::Logoutrequest.new.create(settings, { RelayState: 'http://example.com' })

      assert_includes unauth_url, '&RelayState=http%3A%2F%2Fexample.com'
    ...
    describe 'when the target url contains a query string' do
      it 'creates the SAMLRequest parameter correctly' do
        ...
        assert_match(%r{^http://example\.com\?field=value&SAMLRequest}, unauth_url)
      end
    end

    describe 'consummation of logout may need to track the transaction' do
      it 'has access to the request uuid' do
        settings.idp_slo_service_url = 'http://example.com?field=value'
        ...
        assert_match(/ID="#{unauth_req.uuid}"/, inflated)
      end
    end
