  ...
  describe 'Response' do
    ...
    describe '#validate_signature' do
      ...
      it 'return false when no X509Certificate and no cert provided in settings' do
        ...
        refute response_valid_signed_without_x509certificate.send(:validate_signature)
      ...
      it 'return false when cert expired and check_idp_cert_expiration enabled' do
        ...
        refute response_valid_signed.send(:validate_signature)
      ...
      it 'return false when X509Certificate has invalid content' do
        ...
        refute response_invalid_x509certificate.send(:validate_signature)
      ...
      it 'return false when signature wrapping attack' do
        ...
        refute response_wrapped.send(:validate_signature)
    ...
    describe '#validate nameid' do
      it 'return false when no nameid element and required by settings' do
        ...
        refute response_no_nameid.send(:validate_name_id)
      ...
      it 'return false when no nameid element and required by settings' do
        ...
        refute response_empty_nameid.send(:validate_name_id)
      ...
      it 'return false when wrong_spnamequalifier' do
        ...
        refute response_wrong_spnamequalifier.send(:validate_name_id)
    ...
    describe '#check_one_conditions' do
      it 'return false when none or more than one conditions element' do
        ...
        refute response_no_conditions.send(:validate_one_conditions)
    ...
    describe '#check_one_authnstatement' do
      it 'return false when none or more than one authnstatement element' do
        ...
        refute response_no_authnstatement.send(:validate_one_authnstatement)
    ...
    describe '#check_conditions' do
      it 'check time conditions' do
        ...
        refute response.send(:validate_conditions)
    ...
    describe '#attributes' do
      ...
      it 'return false when validating a response with duplicate attributes' do
        ...
        refute response_duplicated_attributes.send(:validate_no_duplicated_attributes)
      ...
      describe '#multiple values' do
        ...
        it 'extract first of multiple values as string for b/w compatibility in compatibility mode off' do
          ...
          assert_equal %w[value1 value2], response_multiple_attr_values.attributes[:another_value]
          RubySaml::Attributes.single_value_compatibility = true
        end

        it 'return array with all attributes when asked in XML order' do
          assert_equal %w[value1 value2], response_multiple_attr_values.attributes.multi(:another_value)
        end
        ...
        it 'return first of multiple values when multiple Attribute tags in XML in compatibility mode off' do
          ...
          assert_equal %w[role1 role2 role3], response_multiple_attr_values.attributes[:role]
          RubySaml::Attributes.single_value_compatibility = true
        end

        it 'return all of multiple values in reverse order when multiple Attribute tags in XML' do
          assert_equal %w[role1 role2 role3], response_multiple_attr_values.attributes.multi(:role)
        end
        ...
        it 'return all of multiple values when multiple Attribute tags in multiple AttributeStatement tags' do
          ...
          assert_equal %w[role1 role2 role3], response_with_multiple_attribute_statements.attributes.multi(:role)
          RubySaml::Attributes.single_value_compatibility = true
        end
        ...
        it 'check what happens when trying to retrieve attribute that does not exists' do
    ...
    describe '#session_expires_at' do
      it 'extract the value of the SessionNotOnOrAfter attribute' do
        assert_kind_of Time, response.session_expires_at
      end
    ...
    describe '#sign_document' do
      it 'sign an unsigned SAML Response XML and initiate the SAML object with it' do
        ...
        Timecop.freeze(Time.parse('2015-03-18T04:50:24Z')) do
          assert_predicate signed_response, :is_valid?
        end
    ...
    describe '#want_assertion_signed' do
      ...
      it 'returns false if :want_assertion_signed enabled and Assertion not signed' do
        refute @no_signed_assertion.send(:validate_signed_elements)
    ...
    describe 'retrieve nameID' do
      it 'is possible  when nameID is inside the assertion' do
        response_valid_signed.settings = settings

        assert_equal 'test@onelogin.com', response_valid_signed.nameid
      end

      it 'is not possible when encryptID is inside the assertion but no private key' do
      ...
      it 'is possible when encryptID is inside the assertion and settings has the private key' do
        settings.private_key = ruby_saml_key_text
        response_encrypted_nameid.settings = settings
    ...
    describe 'retrieve nameID and attributes from encrypted assertion' do
      ...
      it 'is possible when signed_message_encrypted_unsigned_assertion' do
        ...
        Timecop.freeze(Time.parse('2015-03-19T14:30:31Z')) do
          assert_predicate response, :is_valid?
          assert_empty response.errors
    ...
    describe '#decrypt_assertion' do
      ...
      describe 'check right settings' do
        it 'is not possible to decrypt the assertion if no private key' do
          ...
          assert_raises(RubySaml::ValidationError, error_msg) do
            RubySaml::XML::Decryptor.decrypt_assertion(response.document, encrypted_assertion_node)
          end
        end

        it 'is not possible to decrypt the assertion if the private key has expired and :check_sp_expiration is true' do
          settings.certificate = ruby_saml_cert_text
        ...
        it 'is possible to decrypt the assertion if the private key is provided' do
          response = RubySaml::Response.new(signed_message_encrypted_unsigned_assertion, settings: settings)
        ...
        it 'is possible to decrypt the assertion if the private key is provided and EncryptedKey RetrievalMethod is present in the response' do
          settings.private_key = ruby_saml_key_text
        ...
        it 'is possible to decrypt the assertion if a private key is provided but there is no SAML namespace on the Assertion Element that is inside the EncryptedAssertion' do
    ...
    describe '#status_code' do
      it 'urn:oasis:names:tc:SAML:2.0:status:Responder' do
        assert_equal 'urn:oasis:names:tc:SAML:2.0:status:Responder', response_statuscode_responder.status_code
      end

      it 'urn:oasis:names:tc:SAML:2.0:status:Requester and urn:oasis:names:tc:SAML:2.0:status:UnsupportedBinding' do
        assert_equal 'urn:oasis:names:tc:SAML:2.0:status:Requester | urn:oasis:names:tc:SAML:2.0:status:UnsupportedBinding', response_double_statuscode.status_code
      end
    end
    ...
    each_signature_algorithm do |idp_key_algo, idp_hash_algo|
      describe '#validate_signature' do
        ...
        it 'raise when invalid signature' do
          ...
          assert_includes response.errors, 'Key validation error'
        end
