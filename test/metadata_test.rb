            ...
            let(:subclass) do
              Class.new(RubySaml::Metadata) do
                def add_extras(root, _settings)
                  idp = Nokogiri::XML::Element.new("md:IDPSSODescriptor", root.document)
                  idp["protocolSupportEnumeration"] = 'urn:oasis:names:tc:SAML:2.0:protocol'

                  nid = Nokogiri::XML::Element.new("md:NameIDFormat", root.document)
                  nid.content = 'urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress'
                  idp.add_child(nid)

                  sso = Nokogiri::XML::Element.new("md:SingleSignOnService", root.document)
                  sso["Binding"] = 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST'
                  sso["Location"] = 'https://foobar.com/sso'
                  idp.add_child(sso)
                  root.add_child(idp)

                  org = Nokogiri::XML::Element.new("md:Organization", root.document)
                  org_name = Nokogiri::XML::Element.new("md:OrganizationName", root.document)
                  org_name["xml:lang"] = "en-US"
                  org_name.content = 'ACME Inc.'
                  org.add_child(org_name)

                  org_display = Nokogiri::XML::Element.new("md:OrganizationDisplayName", root.document)
                  org_display["xml:lang"] = "en-US"
                  org_display.content = 'ACME'
                  org.add_child(org_display)

                  org_url = Nokogiri::XML::Element.new("md:OrganizationURL", root.document)
