  ...
  context "Logoutresponse" do
    context "#new" do
      ...
      should "default to empty settings" do
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse( valid_response)
        assert logoutresponse.settings.nil?
      end
      should "accept constructor-injected settings" do
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse(valid_response, settings)
        assert !logoutresponse.settings.nil?
      end
      should "accept constructor-injected options" do
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse(valid_response, nil, { :foo => :bar} )
        assert !logoutresponse.options.empty?
      end
      should "support base64 encoded responses" do
        expected_response = valid_response
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse(Base64.encode64(expected_response), settings)
    ...
    context "#validate" do
      should "validate the response" do
        ...
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse(valid_response({:uuid => in_relation_to_request_id}), settings)
      ...
      should "invalidate responses with wrong id when given option :matches_uuid" do
        ...
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse(valid_response, settings, opts)
      ...
      should "invalidate responses with wrong request status" do
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse(unsuccessful_response, settings)
    ...
    context "#validate!" do
      ...
      should "raise validation error when in bad state" do
        # no settings
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse(unsuccessful_response)
        assert_raises(OneLogin::RubySaml::ValidationError) { logoutresponse.validate! }
      end
      ...
      should "raise validation error when in lack of issuer setting" do
        ...
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse(unsuccessful_response, bad_settings)
        assert_raises(OneLogin::RubySaml::ValidationError) { logoutresponse.validate! }
      end

      should "raise error for invalid xml" do
        logoutresponse = OneLogin::RubySaml::Logoutresponse.parse(invalid_xml_response, settings)
